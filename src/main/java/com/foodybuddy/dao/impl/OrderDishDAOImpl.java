...
public class OrderDishDAOImpl implements OrderDishDAO {
	...
	public OrderDish getByDishId(Integer DishId) {
		OrderDish orderDish = (OrderDish) session.createQuery("from Order_dish where Order_dish.dish_id =" + DishId)
				.list().get(0);
		return orderDish;
	}
	...
	public OrderDish getByOrderId(Integer OrderId) {
		OrderDish orderDish = (OrderDish) session.createQuery("from Order_dish where Order_dish.order_id =" + OrderId)
				.list().get(0);
		return orderDish;
	}
	...
	public Integer sumDishPrice(Integer OrderId) {
		Integer netDishPrice = (Integer) session
				.createQuery("SELECT SUM(net_dish_price) FROM order_dish GROUP BY order_id").uniqueResult();
		return netDishPrice;
	}
